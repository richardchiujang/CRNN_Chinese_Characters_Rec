...
def parse_arg():
    ...
    parser.add_argument('--checkpoint', type=str, default='output/360CC/checkpoints/checkpoint_350_acc_0.9743.pth',
...
if __name__ == '__main__':
    ...
    started = time.time()
    for imgn in os.listdir(args.image_path):
        img = cv2.imread(os.path.join(args.image_path, imgn))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        sim_pred = recognition(config, img, model, converter, device)
        print('results: {} {}'.format(sim_pred, imgn))
